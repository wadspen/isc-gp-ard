...
parc_df <- parc_df %>%
  mutate(
    xbin = cut(x,
               breaks = seq(min(x) - 1, max(x), 
                            length.out = ceiling(max(x)/bin_wid) + 1),  # 12 equal bins
               labels = FALSE,
               include.lowest = TRUE),
    ybin = cut(y,
               breaks = seq(min(y) - 1, max(y),
                            length.out = ceiling(max(y)/bin_wid) + 1),  # 11 equal bins
               labels = FALSE,
               include.lowest = TRUE),
...
y_breaks <- quantile(parc_df$y, probs = seq(0, 1, length.out = 11))[-c(1,9)]

rois <- unique(parc_df$roi)
parc_df %>% 
  # filter(roi == 13) %>% 
  ggplot( aes(x, y, fill = factor(roi, levels = sample(rois, length(rois))))) +
