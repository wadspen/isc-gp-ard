...
est_cor <- matrix(NA, nrow = 53, ncol = 63)
for (c in 1:nrow(coords)) {
  ...
  for (i in 1:length(subjects)) {
    for (j in i:length(subjects)) {
      sub_cor[i,j] <- as.numeric(cor(scale(sub_data[[i]][x,y,]), 
                                     scale(sub_data[[j]][x,y,])))
      sub_cor[j,i] <- sub_cor[i,j]
      
    }
  }
  diag(sub_cor) <- 0
  est_cor[x, y] <- mean(abs(sub_cor), na.rm = TRUE)
...
hdr_sim %>% 
  ...
  unique() %>% 
  group_by(time) %>% 
  mutate(mhdr = mean(hdr)) %>% 
  ggplot() +
  geom_line(aes(x = time, y = hdr, colour = factor(subject))) +
  geom_line(aes(x = time, y = mhdr), size = 1.2) +
  theme_bw() +
...
mx <- dim(est_cor)[1]
my <- dim(est_cor)[2]
ec_df <- data.frame(expand.grid(xc = 1:mx, yc = 1:my),
                    corr = as.vector(est_cor))


ec_df %>% 
  mutate(corr = ifelse(xc == 15 & yc == 12, .13, corr)) %>%
  # mutate(corr = ifelse(xc == 46 & yc == 16, .13, corr)) %>%
  ggplot(aes(x = xc, y = yc, fill = corr)) +
  geom_tile() +
